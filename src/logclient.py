...
class LogConf(object):
    ...
    def __init__(self, filename='flaxlog.conf'):
...
class LogConfPub(object):
    """ Publishes changes in the file named by `filename` to all the
    objects in subscribers. These must have a .send method which will
    be called to send the new file contents to them.
    """
    def __init__(self, filename, subscribers):
        self.filename = filename
        self.subscribers = subscribers
        util.FileWatcher(self.filename, self.publish_new_file).start()
